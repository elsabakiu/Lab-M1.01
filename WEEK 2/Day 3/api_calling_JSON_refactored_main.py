...
# Builds the request body for one listing API call.
...
# Sends one request to OpenAI and returns text output.
def request_listing_text(
    client: OpenAI,
    ...
    try:
        ...
        report_error(
            function_name="request_listing_text",
            error=error,
            location=f"model={model}",
            suggestion="Check internet access, API key, quota, and rate limits.",
        )
        raise
    ...
    if not raw_text:
        ...
        report_error(
            ...
            error=error,
            location=f"model={model}",
            suggestion="Retry request or reduce prompt complexity.",
        )
        raise error
    return raw_text


# Full API helper: build request and fetch response text.
def fetch_listing_text(
    ...
    return request_listing_text(client, api_input, model, temperature)
